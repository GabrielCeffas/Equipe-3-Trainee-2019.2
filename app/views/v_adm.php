

    <div class="container text-center">
        <div class="mt-4">
            <h1>Bem Vindo a Home Administrativa!</h1>
        </div>
        
        <div class="form-row container mt-5">
            <div class="mt-3 col-md-4 mb-3">
                <a title="Usuários" href="<?php echo URL_BASE ."/usuario"?>"><i class="fas fa-users adm"></i></a>
                <p><h3>Usuários</h3></p>
            </div>
            <div class="mt-3 col-md-4 mb-3">
                <a title="Produtos" href="<?php echo URL_BASE ."/produto"?>"><i class="fas fa-gem adm"></i></a>
                <p><h3>Produtos</h3></p>
            </div>
            <div class="mt-3 col-md-4 mb-3">
                <a title="Categorias" href="<?php echo URL_BASE ."/categoria"?>"><i class="fas fa-list adm"></i></a>
                <p><h3>Categorias</h3></p>
            </div>
        </div>
        <div class="mt-5 col-md-4 mb-3">
            <button type="button" class="btn btn-outline-dark"><i class="fas fa-arrow-left"></i> Sair da Página Administrativa</button>
        </div>
    </div>
    
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
